...
export class WikiIndexStore {
  ...
  async rebuild(input: { pages: WikiPage[]; rawDocuments: RawDocument[] }): Promise<void> {
    const db = this.db();
    db.exec("DELETE FROM page_links; DELETE FROM source_refs; DELETE FROM wiki_fts; DELETE FROM pages; DELETE FROM raw_documents;");
    ...
    for (const page of input.pages) {
      ...
      insertFts.run(page.id, page.title, [page.summary, page.body, page.tags.join(" "), page.aliases.join(" ")].join("\n"));
    }
...
function fallbackSearch(query: string, limit: number, pages: WikiPage[], rawById: Map<string, RawDocument>): WikiSearchResult[] {
  ...
  return pages
    .map((page) => {
      const haystack = [page.title, page.summary, page.body, page.tags.join(" "), page.aliases.join(" ")].join("\n").toLowerCase();
